...
'''
    Word analogy python3 program -> that solves analogies such as "dog is to cat as puppy is to ___". using Stanford's Glove model.
...
# Converting glove word to vec txt file to word2vec model for using faster buitin functionality of gensim model
glove2word2vec(glove_input_file=words_vector_file, word2vec_output_file='temp_glove_model.word2vec')

# load the Stanford's Glove model of 100 dim word2vec format
model = KeyedVectors.load_word2vec_format(fname='temp_glove_model.word2vec', binary=False)
...
'''
    ...
    - Learned how to use pre-trained stanford's pre-trained word to vector model.
